...
def scrape_dictionary(letters=None, output_file="dizionario_completo.jsonl", generate_report=False, analysis=False):
    ...
    if analysis:
        with open(output_file, 'a', encoding='utf-8') as f:
            for i, link in enumerate(word_links):
                print(f"[{uppercase_letter}] Analisi {i+1}/{len(word_links)}: {link}")
...
if __name__ == "__main__":
    scrape_dictionary(letters=['z'], output_file="dizionario_z.jsonl")
    # scrape_dictionary()
